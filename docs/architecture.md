...
(`--section-py`) + тон фона + `Container` внутри. Это канон для **будущих** страниц
(проекты, блог). `ProjectCard.astro` пока живёт в `/styleguide` — станет боевым в
разделе проектов (Фаза 2).
...
Проект на строгом пресете (`astro/tsconfigs/strict`). Доменные данные типизированы
интерфейсами в [`src/types.ts`](../src/types.ts) и проверяются через `satisfies`
в data-файлах: [`home.ts`](../src/data/home.ts) — контент главной,
[`site.ts`](../src/data/site.ts) — сквозные `contact`/`nav` (их потребляют
`Header`, `Footer`, `CtaCalc`). Опечатка в данных ловится на сборке.
...
проверки гоняет CI-гейт `quality` перед деплоем (см. [deployment.md](deployment.md)).
